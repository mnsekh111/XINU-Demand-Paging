...
/* array in the code)		*/
{
	...
	// New frame for page directory
	int new_frame_pd;

	get_frm(&new_frame_pd);

	frm_tab[new_frame_pd].fr_type = FR_DIR;
	frm_tab[new_frame_pd].fr_pid = pid;


	pptr->pdbr = (new_frame_pd+FRAME0)*NBPG;
	pptr->store = -1;
	pptr->vhpno = -1;
	pptr->vhpnpages = -1;

	pd_t *dir_entry = (pd_t*)pptr->pdbr;
	pd_t *glob_dir_entry = (pd_t*)proctab[NULLPROC].pdbr;

	for(i=0;i<NGPT;i++){

		init_pd_entry(dir_entry);
		dir_entry++->pd_base = glob_dir_entry++->pd_base;
